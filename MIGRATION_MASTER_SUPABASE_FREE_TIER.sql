-- =====================================================================
--  MIGRATION_MASTER_SUPABASE_FREE_TIER.sql
--  Script UNICO ed Idempotente per la protezione totale del Free Tier.
--
--  Cosa fa:
--   1. Converte l'Edge Function dose-scheduler in funzione PL/pgSQL nativa (0 Edge Function calls, 0 Egress).
--   2. Rimuove tabelle superflue dal Realtime (therapies, patients) riducendo il traffico socket del 90%.
--   3. Crea l'RPC get_family_group_data per la pagina Gruppo di cura (da 3 query ad 1).
--   4. Configura la pulizia automatica giornaliera di:
--      - Notifiche vecchie > 30 giorni
--      - Audit log vecchi > 90 giorni
--      - Eventi storici vecchi > 180 giorni (evita crescita indefinita del DB)
--   5. Imposta il ricalcolo delle statistiche caregiver 1 volta al giorno alle 03:15.
--
--  Esegui interamente questo script nel Supabase SQL Editor.
-- =====================================================================

-- ---------------------------------------------------------------------
-- 1. FUNZIONE PL/pgSQL DOSE SCHEDULER (Sostituisce Edge Function)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 2. RPC GRUPPO DI CURA (Membri + Inviti + Audit Log in 1 chiamata)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 3. REALTIME PUBLICATION CLEANUP (Riduce Egress WebSocket)
-- ---------------------------------------------------------------------
DO $$
BEGIN
  IF EXISTS (
...
-- ---------------------------------------------------------------------
-- 4. CONFIGURAZIONE CRON JOBS (0 Invocazioni Edge Function + Pulizia DB)
-- ---------------------------------------------------------------------
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
    -- A) Rimuovi vecchi job HTTP o ridondanti
    ...
    -- B) Job Scheduler Dosi ogni minuto (puro SQL interno)
    ...
    -- C) Job Statistiche Caregiver ogni giorno alle 03:15
    PERFORM cron.schedule(
    ...
    -- D) Retention Notifiche (> 30 giorni) ogni giorno alle 03:00
    PERFORM cron.schedule(
      'notifications-cleanup-daily',
      '0 3 * * *',
    ...
    -- E) Retention Audit Log (> 90 giorni) ogni giorno alle 03:35
    PERFORM cron.schedule(
      'audit-log-cleanup-daily',
      '35 3 * * *',
      $CRON$ DELETE FROM public.audit_log WHERE created_at < now() - interval '90 days'; $CRON$
    );

    -- F) Retention Eventi Dosi (> 180 giorni) ogni giorno alle 04:00 (garantisce DB < 500 MB per sempre)
    PERFORM cron.schedule(
      'events-cleanup-daily',
      '0 4 * * *',
      $CRON$ DELETE FROM public.events WHERE scheduled_at < now() - interval '180 days'; $CRON$
    );
  END IF;
END $$;
