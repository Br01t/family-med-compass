-- FamilyMed — notifiche v2 (idempotente)
-- Da eseguire una volta nel SQL editor del tuo Supabase esterno.
--
-- Cosa fa:
--  * Aggiorna handle_dose_taken(): distingue "taken" vs "taken_after_snooze",
--    include l'orario della dose (a quale modale si riferisce l'azione),
--    e disattiva la generazione di low_stock per singola dose
--    (rimane comunque possibile ricrearla come alert giornaliero separato).
--  * Aggiorna handle_dose_status_change(): testi caregiver che citano
--    l'orario della dose di riferimento, aggiunge minuti di snooze,
--    e migliora il testo del "missed" per il paziente.
--  * Ricrea i trigger su events in modo idempotente.
--  * Mantiene l'index unico su (target_user_id, dose_key).

-- ============================================================
-- A) handle_dose_taken — usato dal trigger trg_dose_taken
-- ============================================================
...
BEGIN
  ...
  -- Notifica caregiver: azione paziente su modale delle HH:MM
  FOR v_caregiver IN
  ...
  -- Scorta bassa: alert giornaliero per caregiver (una sola volta per giorno per terapia)
...
-- ============================================================
-- B) handle_dose_status_change — snoozed / skipped / missed
-- ============================================================
...
BEGIN
  ...
  ELSIF NEW.status = 'skipped' THEN
    ...
    v_pt_msg   := 'Dose delle ' || v_hhmm || ' contrassegnata come saltata.';
  ELSE
  ...
  -- Notifica paziente per le proprie azioni (skipped/snoozed) e per missed.
  -- Le notifiche reminder_pre/due/reminder_post/final_due/missed dallo scheduler
  -- sono generate dalla edge function dose-scheduler.
  IF v_patient.user_id IS NOT NULL THEN
    IF NEW.status IN ('skipped','snoozed') THEN
      INSERT INTO public.notifications
        (target_user_id, kind, severity, title, message, patient_id, therapy_id, event_id, dose_key)
      VALUES (
      ...
      ON CONFLICT DO NOTHING;
    END IF;
    -- La notifica 'missed' per il paziente è già inserita dallo scheduler
    -- con testo dedicato ("probabilmente verrai contattato..."). Se lo status
    -- viene aggiornato via UI e lo scheduler non l'ha già inserita, aggiungila.
    IF NEW.status = 'missed' THEN
      INSERT INTO public.notifications
        (target_user_id, kind, severity, title, message, patient_id, therapy_id, event_id, dose_key)
      VALUES (
        v_patient.user_id, 'missed', 'alert', v_pt_title, v_pt_msg,
        NEW.patient_id, NEW.therapy_id, NEW.id,
        NEW.therapy_id || '@' || NEW.scheduled_at::text || '@missed@patient'
      )
      ON CONFLICT DO NOTHING;
    END IF;
  END IF;
...
-- ============================================================
-- D) (Opzionale) Cron ogni minuto per lo scheduler
-- ============================================================
