...
create policy "patients: insert self or as caregiver" on public.patients for insert to authenticated
  with check (user_id=auth.uid() or (public.has_role(auth.uid(),'caregiver') and (owner_user_id is null or owner_user_id=auth.uid())));

-- CAREGIVERS
create table public.caregivers (
  id uuid primary key references auth.users(id) on delete cascade,
  name text, relation text, photo text,
...
create policy "cp: read own" on public.caregiver_patients for select to authenticated
  using (caregiver_id=auth.uid() or exists(select 1 from public.patients p where p.id=patient_id and p.user_id=auth.uid()));
create policy "cp: caregiver can follow" on public.caregiver_patients for insert to authenticated
  with check (caregiver_id=auth.uid() and public.has_role(auth.uid(),'caregiver'));
create policy "cp: caregiver can update own" on public.caregiver_patients for update to authenticated
...
create policy "patients: owner or linked caregiver update" on public.patients for update to authenticated
  using (user_id=auth.uid() or owner_user_id=auth.uid()
    or exists(select 1 from public.caregiver_patients cp where cp.patient_id=patients.id and cp.caregiver_id=auth.uid()));
create policy "patients: owner or linked caregiver delete" on public.patients for delete to authenticated
  using (user_id=auth.uid() or owner_user_id=auth.uid()
    or exists(select 1 from public.caregiver_patients cp where cp.patient_id=patients.id and cp.caregiver_id=auth.uid()));
create policy "profiles: caregiver can read followed patients" on public.profiles for select to authenticated
  using (exists(select 1 from public.caregiver_patients cp join public.patients p on p.id=cp.patient_id
                where cp.caregiver_id=auth.uid() and p.user_id=profiles.id));
create policy "caregivers: patient can read linked" on public.caregivers for select to authenticated
  using (exists(select 1 from public.caregiver_patients cp join public.patients p on p.id=cp.patient_id
...
create table public.therapies (
  ...
  category text, color text, icon text, notes text,
  ...
  photo_drug text, photo_package text,
  created_at timestamptz not null default now()
);
...
create policy "therapies: rw se paziente o caregiver linked" on public.therapies for all to authenticated
  using (exists(select 1 from public.patients p where p.id=patient_id and (p.user_id=auth.uid() or p.owner_user_id=auth.uid()
    or exists(select 1 from public.caregiver_patients cp where cp.patient_id=p.id and cp.caregiver_id=auth.uid()))))
  with check (exists(select 1 from public.patients p where p.id=patient_id and (p.user_id=auth.uid() or p.owner_user_id=auth.uid()
    or exists(select 1 from public.caregiver_patients cp where cp.patient_id=p.id and cp.caregiver_id=auth.uid()))));
...
create policy "events: rw se paziente o caregiver linked" on public.events for all to authenticated
  using (exists(select 1 from public.patients p where p.id=patient_id and (p.user_id=auth.uid() or p.owner_user_id=auth.uid()
    or exists(select 1 from public.caregiver_patients cp where cp.patient_id=p.id and cp.caregiver_id=auth.uid()))));
...
create policy "stock: read se accesso alla terapia" on public.stock_movements for select to authenticated
  using (exists(select 1 from public.therapies t join public.patients p on p.id=t.patient_id
    where t.id=therapy_id and (p.user_id=auth.uid() or p.owner_user_id=auth.uid()
      or exists(select 1 from public.caregiver_patients cp where cp.patient_id=p.id and cp.caregiver_id=auth.uid()))));
create policy "stock: insert se accesso alla terapia" on public.stock_movements for insert to authenticated
  with check (exists(select 1 from public.therapies t join public.patients p on p.id=t.patient_id
    where t.id=therapy_id and (p.user_id=auth.uid() or p.owner_user_id=auth.uid()
      or exists(select 1 from public.caregiver_patients cp where cp.patient_id=p.id and cp.caregiver_id=auth.uid()))));
...
create unique index notifications_dose_key_idx on public.notifications(target_user_id, dose_key) where dose_key is not null;
create index notifications_target_idx on public.notifications(target_user_id, read);
grant select, insert, update on public.notifications to authenticated;
grant all on public.notifications to service_role;
alter table public.notifications enable row level security;
create policy "notifications: read own or caregiver of patient" on public.notifications for select to authenticated
  using (target_user_id=auth.uid() or (patient_id is not null and exists(
    select 1 from public.patients p where p.id=notifications.patient_id and (p.owner_user_id=auth.uid()
      or exists(select 1 from public.caregiver_patients cp where cp.patient_id=p.id and cp.caregiver_id=auth.uid())))));
create policy "notifications: insert if linked to patient" on public.notifications for insert to authenticated
  with check (patient_id is null or exists(select 1 from public.patients p where p.id=patient_id and (
    p.user_id=auth.uid() or p.owner_user_id=auth.uid()
    or exists(select 1 from public.caregiver_patients cp where cp.patient_id=p.id and cp.caregiver_id=auth.uid()))));
...
declare v_name text; v_role app_role; v_patient_id text;
begin
  v_name := coalesce(new.raw_user_meta_data->>'name', new.email);
  v_role := coalesce((new.raw_user_meta_data->>'role')::app_role, 'caregiver');
  insert into public.profiles (id, email, name, role) values (new.id, new.email, v_name, v_role)
  ...
  if v_role='caregiver' then
    insert into public.caregivers (id, name) values (new.id, v_name) on conflict (id) do nothing;
  end if;
  if v_role='paziente' then
    v_patient_id := 'p_'||new.id::text;
    insert into public.patients (id, name, user_id) values (v_patient_id, v_name, new.id) on conflict (id) do nothing;
  end if;
  return new;
end; $$;
...
returns trigger language plpgsql security definer set search_path=public as $$
declare v_therapy public.therapies%rowtype; v_patient public.patients%rowtype; v_caregiver uuid;
begin
  ...
  update public.therapies set pills_remaining=greatest(0, pills_remaining-coalesce(v_therapy.quantity,1)) where id=new.therapy_id;
...
create trigger on_event_taken after insert or update of status on public.events for each row execute function public.handle_dose_taken();
