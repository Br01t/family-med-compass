-- ============================================================
-- OTTIMIZZAZIONE FAMELYMED DOSE SCHEDULER & LIMITI FREE TIER
-- ============================================================
-- 1. Indici parziali ultraleggeri per Seek istantaneo (<1ms)
-- 2. Finestra reminder_pre ridotta da 24h a 20min
-- 3. Generazione dosi a 24h solo ogni 15 minuti o se vuota
--    (riduce del 95% le scritture su DB e il consumo CPU)
-- ============================================================

-- 1. INDICI PARZIALI DEDICATI
CREATE INDEX IF NOT EXISTS idx_events_active_schedule
  ON public.events (scheduled_at)
  WHERE status IN ('scheduled', 'snoozed');

CREATE INDEX IF NOT EXISTS idx_events_snoozed_until
  ON public.events (snoozed_until)
  WHERE status = 'snoozed' AND snoozed_until IS NOT NULL;


-- 2. FUNZIONE PROCESS_DOSE_SCHEDULE ULTRA-OTTIMIZZATA
...
BEGIN
  ...
  -- Viene eseguita ogni 15 minuti (:00, :15, :30, :45) oppure se non ci sono eventi
  -- nell'ora successiva. Questo evita di tentare centinaia di INSERT ogni 60 secondi.
  ...
  -- 2a) REMINDER_PRE (Finestra mirata di 20 minuti invece di 24 ore!)
  FOR v_ev IN
  ...
  -- 2b) DUE (Finestra stretta: orario esatto)
  FOR v_ev IN
    SELECT e.id, e.therapy_id, e.patient_id, e.scheduled_at,
  ...
  -- 2c) REMINDER_POST (+5 min dall'orario)
  FOR v_ev IN
  ...
  -- 2e) MISSED (Superamento deadline)
  FOR v_ev IN
    ...
    WHERE e.status IN ('scheduled', 'snoozed')
      AND e.scheduled_at BETWEEN (v_now - interval '24 hours') AND (v_now - interval '5 minutes')
      AND p.suspended_at IS NULL
      AND t.suspended_at IS NULL
  LOOP
