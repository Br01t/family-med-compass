...
-- ============================================================
-- CRON JOBS (pg_cron) — non esportati da pg_dump
-- ============================================================

SELECT cron.schedule('vital-signs-cleanup-daily',                '45 3 * * *',  'SELECT public.cleanup_vital_signs();');
SELECT cron.schedule('refresh-caregiver-dashboard-stats-daily',  '15 3 * * *',  'SELECT public.refresh_caregiver_dashboard_stats();');
SELECT cron.schedule('familymed-dose-scheduler',                 '* * * * *',   'SELECT public.process_dose_schedule();');
SELECT cron.schedule('notifications-cleanup-daily',              '0 3 * * *',   'DELETE FROM public.notifications WHERE created_at < now() - interval ''30 days'';');
SELECT cron.schedule('events-cleanup-daily',                     '0 4 * * *',   'DELETE FROM public.events WHERE scheduled_at < now() - interval ''180 days'';');
SELECT cron.schedule('adherence-monthly-rollup',                 '0 3 2 * *',   'SELECT public.rollup_adherence_monthly();');
SELECT cron.schedule('daily-gdpr-events-cleanup',                '0 3 * * *',   'SELECT public.cleanup_expired_events();');
SELECT cron.schedule('daily-gdpr-cleanup',                       '0 3 * * *',   'SELECT public.cleanup_expired_records();');
SELECT cron.schedule('audit-log-cleanup-daily',                  '35 3 * * *',  'DELETE FROM public.audit_log WHERE created_at < now() - interval ''90 days'';');
SELECT cron.schedule('wellness-notes-cleanup-daily',             '40 3 * * *',  'DELETE FROM public.wellness_notes WHERE occurred_at < now() - interval ''24 months'';');


-- ============================================================
-- STORAGE POLICIES — non esportate da pg_dump
-- ============================================================

CREATE POLICY "therapy_photos_primary_delete"
  ON storage.objects AS permissive FOR DELETE TO authenticated
  USING (
    (bucket_id = 'therapy-photos'::text)
    AND ((storage.foldername(name))[1] = 'therapies'::text)
    AND (EXISTS (
      SELECT 1 FROM public.therapies t
      WHERE (t.id = (storage.foldername(t.name))[2])
...
CREATE POLICY "therapy_photos_public_read"
  ...
  USING (bucket_id = 'therapy-photos'::text);
